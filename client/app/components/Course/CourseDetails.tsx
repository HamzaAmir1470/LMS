...
const CourseDetails = ({
  ...
  // Added refetch capability and proper query configuration
  const { data: userData, refetch } = useLoadUserQuery(undefined, {
    refetchOnMountOrArgChange: true,
  });

  // Handle user data updates and auto-stop loading
  useEffect(() => {
    if (userData?.user) {
      setUser(userData.user);

      // Check if the course is now purchased
      const hasPurchased = userData.user?.courses?.some(
        (item: any) => item.courseId === data?._id,
      );

      // If purchased and still loading, stop loading
      if (hasPurchased && isLoading) {
        setIsLoading(false);
      }
    }
  }, [userData, data?._id, isLoading]);

  // Cleanup on unmount to prevent loading state persistence
  useEffect(() => {
    return () => {
      setIsLoading(false);
    };
  ...
  return (
    <>
      {/* FIX: Instead of full DOM replacement, render the loader as an overlay.
        This keeps the CheckoutForm mounted in the background so its redirection useEffect can execute.
      */}
      {isLoading && (
      ...
      <div className="w-[90%] 800px:w-[90%] m-auto py-5">
        <div className="w-full flex flex-col-reverse 800px:flex-row gap-5">
          ...
          {/* Right Side: Floating Sticky Sidebar */}
        ...
        {open && (
          <div className="w-full h-screen bg-[#0000005d] fixed top-0 left-0 z-[9999] flex items-center justify-center p-3 sm:p-4">
            <div className="w-full max-w-[500px] min-h-[400px] bg-white dark:bg-slate-900 rounded-xl shadow-2xl p-5 relative flex flex-col justify-between overflow-hidden">
              {/* Close Button Row */}
              ...
              {/* Payment Container */}
              <div className="w-full my-auto text-black dark:text-white">
                {stripePromise && clientSecret ? (
                  ...
                  <div className="flex flex-col gap-2 justify-center items-center h-[200px]">
                    <span className="text-black dark:text-white font-Poppins animate-pulse">
